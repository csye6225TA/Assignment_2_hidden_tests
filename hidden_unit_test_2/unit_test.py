@patch('requests.post')
def test_load_data_failure(self, mock_post):
   ...
   # Attempt to load data from a snapshot with an invalid ID
    response = client.load_data('invalid-id')
    ...
    mock_post.assert_called_once_with('http://localhost:5000/load_data', headers={'Content-Type': 'application/json'}, data=json.dumps({"snapshot_id": "invalid-id"}))
